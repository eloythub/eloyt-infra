...
const projectId = 'e-stock-182503'
const hostname = 'gcr.io'
const tag = `${env}-${tagVersion}`

const manifestDir  = path.resolve(path.join(__dirname, 'kube', '.deploy-manifests', hostname, env, service))
...
const dockerGcloudImage = `${service}`
...
async function createImage () {
  ...
  try {
    ...
    await shell('docker', ['tag', `${dockerLocalImage}:${tag}`, `${hostname}/${projectId}/${dockerGcloudImage}:${tag}`], { cwd })
    await shell('gcloud', ['docker', '--', 'push', `${hostname}/${projectId}/${dockerGcloudImage}:${tag}`], { cwd })
  } catch (err) {
...
async function updateManifest () {
  ...
  const manifestData = {
    image: `${hostname}/${projectId}/${dockerGcloudImage}:${tag}`,
    tag: tag,
    uri: `${hostname}/${projectId}/${dockerGcloudImage}`
  }
